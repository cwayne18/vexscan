...
`--haul` always produces a [batch document](#scanning-a-fleet---images-from),
...
| `--images-from` | | Scan every image named in this list — a file with one reference per line, a [hauler manifest](#hauler-manifests-as-a-list---images-from), a URL, or `-` for stdin. `#` comments allowed, repeats scanned once — see [Scanning a fleet](#scanning-a-fleet---images-from) |
| `--haul` | | Scan every image inside a [hauler haul](#scanning-a-hauler-haul---haul) without a registry — a `.tar.zst`, a tar, or an unpacked store directory. Charts and files in the haul are counted and named on stderr, never dropped silently |
